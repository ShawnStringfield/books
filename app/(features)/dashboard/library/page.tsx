...
export default function LibraryPage() {
  ...
  const errorMessage = error instanceof Error ? error.message : 'An error occurred while loading books';
  ...
  return (
    <DashboardLayout title="My Library">
      <div className={`max-w-4xl mx-auto p-6 ${showEmptyState ? 'h-[calc(100vh-80px)] flex flex-col' : ''}`}>
        {hasBooks && (
          <div className="mb-6">
        ...
        {isLoading ? (
          ...
          <div className={!hasBooks ? 'flex-1 flex items-center justify-center' : ''}>
            <BooksList />
          </div>
        )}
      </div>
