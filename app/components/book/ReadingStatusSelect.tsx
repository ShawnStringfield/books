...
const ReadingStatusSelect = ({ status, onStatusChange, className = '', size = 'default' }: ReadingStatusSelectProps) => {
  const currentOption = readingStatusOptions.find((option) => option.value === status);
        ...
        {readingStatusOptions.map((option) => (
          <SelectItem key={option.value} value={option.value} className={`${size === 'sm' ? 'text-xs' : 'text-sm'} pl-3 pr-2`}>
            <div className="flex items-center gap-2">
              <span className="text-gray-500 w-4 flex justify-center shrink-0">{option.icon}</span>
